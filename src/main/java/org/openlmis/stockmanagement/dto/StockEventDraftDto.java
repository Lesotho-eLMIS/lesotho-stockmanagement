...
public class StockEventDraftDto {
   ...
   */
  public StockEventDraft toEvent() {
    List<StockEventLineItemDraft> domainLines = this.lineItems
        .stream()
        .map(StockEventLineItemDraftDto::toEventLineItem)
        .collect(Collectors.toList());

    StockEventDraft event = new StockEventDraft(facilityId, programId, context.getCurrentUserId(),
    ...
    domainLines.forEach(lineItem -> {
      ...
      lineItem.setStockEventDraft(event);
    });
  ...
  public boolean hasSource() {
    return hasLineItems() && getLineItems()
        .stream()
        .anyMatch(StockEventLineItemDraftDto::hasSourceId);
  }
  ...
  public boolean hasDestination() {
    return hasLineItems() && getLineItems()
        .stream()
        .anyMatch(StockEventLineItemDraftDto::hasDestinationId);
  }
  ...
  public boolean isPhysicalInventory() {
    boolean noReason = hasLineItems()
        && getLineItems().stream().noneMatch(StockEventLineItemDraftDto::hasReasonId);
  ...
  public Set<UUID> getReasonIds() {
    ...
    for (StockEventLineItemDraftDto lineItem : lineItems) {
