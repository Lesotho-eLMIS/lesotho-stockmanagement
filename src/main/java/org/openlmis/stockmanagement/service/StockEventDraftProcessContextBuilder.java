...
@NoArgsConstructor
@AllArgsConstructor
public class StockEventDraftProcessContextBuilder {
  private static final Logger LOGGER = getLogger(StockEventDraftProcessContextBuilder.class);
  private static final XLogger XLOGGER = getXLogger(StockEventDraftProcessContextBuilder.class);
   ...
   */
  public StockEventProcessContext buildContext(StockEventDraftDto eventDto) {
    ...
    List<UUID> orderableIds = eventDto.getLineItems()
        .stream()
        .map(StockEventLineItemDraftDto::getOrderableId)
  ...
  private List<LotDto> getLots(StockEventDraftDto eventDto) {
    return eventDto
        .getLineItems()
        .stream()
        .filter(item -> item.getLotId() != null)
        .map(StockEventLineItemDraftDto::getLotId)
        .distinct()
        .map(lotReferenceDataService::findOne)
        .collect(Collectors.toList());
  }

  private List<StockCardLineItemReason> getCardReasons(StockEventDraftDto eventDto) {
