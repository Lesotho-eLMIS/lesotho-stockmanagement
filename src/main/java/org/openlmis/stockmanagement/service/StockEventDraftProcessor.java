 ...
 */
@Service
public class StockEventDraftProcessor {

  private static final Logger LOGGER = LoggerFactory.getLogger(StockEventDraftProcessor.class);
  private static final XLogger XLOGGER = XLoggerFactory.getXLogger(StockEventDraftProcessor.class);

  @Autowired
  private StockEventDraftProcessContextBuilder contextBuilder;
  ...
  public UUID process(StockEventDraftDto eventDto) {
    ...
    //Disable validation since this is a draft ... some fields may be blank at the time
  ...
  private UUID saveEventAndGenerateLineItems(StockEventDraftDto eventDto, Profiler profiler) {
    profiler.start("CONVERT_TO_EVENT");
    StockEventDraft stockEvent = eventDto.toEvent();

    profiler.start("DB_SAVE");
    UUID savedEventId = stockEventsDraftRepository.save(stockEvent).getId();
